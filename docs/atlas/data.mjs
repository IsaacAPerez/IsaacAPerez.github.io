...
export const NODES = [
  ...
  },
  {
    ...
    one: 'Seven badges that pop as a visitor explores, plus a hidden cheat code — and one more badge in the table that nothing ever awards.',
    ...
    steps: [
      ...
      ['Count', 'game.js syncAchCount() re-reads the store, drops konami, and writes "n/8" into the HUD — 7/8 is the real ceiling.'],
    ...
    cond: [
      { q: '<code>adventurer</code> (🚪 "Stepped Inside") is defined in the badge table at <code>index.html:376</code> but nothing ever calls <code>unlock(\'adventurer\')</code> — <code>js/game.js</code> only unlocks quests / statcheck / lorekeeper / raven / historian / completionist / konami, and <code>index.html</code> adds shifter. Since <code>syncAchCount()</code> counts against a literal 8, the HUD can never read 8/8. Award it on the first <code>openOverlay()</code>, or drop the entry and count against 7?' },
    ],
  },
  {
